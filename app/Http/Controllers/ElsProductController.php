...
class ElsProductController extends Controller
{
    ...
    public function getELSProductPartsDetails(Request $request)
    {
        ...
        if ($data) {
            ...
            })->where('els_order_request.els_system_id', $data->id)
                ->groupBy('els_order_request_parts.id')->select('els_order_request_parts.*', 'els_order_request.els_system_id')->get();
        ...
        if ($order_request && $request->part_id) {
            foreach ($request->part_id as $key => $part_id) {
                ...
                if (isset($request['part_barcode_new'][$id])) {
                    ...
                    if ($fetchDefaultRecord->old_pin) {
                        $old_pin = $fetchDefaultRecord->old_pin . ", " .  $old_pin;
                    }
                    if ($fetchDefaultRecord) {
                        $data = [
                            ...
                            'new_pin' => '1',
                            'barcodes' => $request['part_barcode_new'][$id],
                            'old_pin' => $old_pin,
                            "updated_at" => now()
                        ];
                        ...
                        //also create 'received_purchase_order_parts_list'
            ...
            if ($request->part_id) {
                foreach ($request->part_id as $key => $part_id) {
                    ...
                    $time = time();
                    \App\Models\OrderRequestPart::create([
                        'request_order_id' => $order_request->id,
                        'brand_id' => $brand_id,
